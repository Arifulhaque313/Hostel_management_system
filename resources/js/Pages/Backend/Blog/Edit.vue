<template>
	<Head>
		<title>Blog Edit {{ $page.props.APP_NAME }}</title>
		...
		<div>
			...
			<div class="bg-white rounded-md shadow overflow-hidden max-w-2xl mx-auto px-4 py-5">
				<form @submit.prevent="submit">
					...
					<!-- Thumbnail -->
					<Images v-model:thumbnail="form.thumbnail" v-model:images="form.images" :thumbnail_error="form.errors.thumbnail" :thumbnail_link="blog.thumbnail" :images_link="blog.images.map((image) => image.path)" :images_error="form.errors.images" @deleted_url="deletedImage($event)"> </Images>
					...
					<div class="relative border mt-8 border-gray-200 rounded-sm">
						...
						<div class="relative p-4">
							<!-- test richtext -->
							<textarea type="text" name="description" id="description" class="block w-full pr-10 sm:text-sm rounded-md" rows="4" placeholder="Blog Description" aria-invalid="true" aria-describedby="email-error" v-model="form.description_field_one" />
							<!-- End  test richtext-->
						</div>
					</div>
					...
					<div class="relative border mt-8 border-gray-200 rounded-sm">
						...
						<div class="relative p-4">
							<!-- test richtext -->
							<textarea type="text" name="description" id="description" class="block w-full pr-10 sm:text-sm rounded-md" rows="4" placeholder="Blog Description" aria-invalid="true" aria-describedby="email-error" v-model="form.description_field_two" />
							<!-- End  test richtext-->
						</div>
					</div>
					...
					<div class="relative border mt-8 border-gray-200 rounded-sm">
						...
						<div class="relative p-4">
							<!-- test richtext -->
							<textarea type="text" name="description" id="description" class="block w-full pr-10 sm:text-sm rounded-md" rows="4" placeholder="Blog Description" aria-invalid="true" aria-describedby="email-error" v-model="form.description_field_three" />
							<!-- End  test richtext-->
						</div>
					</div>
					...
					<div class="relative border mt-8 border-gray-200 rounded-sm">
						...
						<div class="relative p-4">
							<!-- test richtext -->
							<textarea type="text" name="description" id="description" class="block w-full pr-10 sm:text-sm rounded-md" rows="4" placeholder="Blog Description" aria-invalid="true" aria-describedby="email-error" v-model="form.description_field_four" />
							<!-- End  test richtext-->
						</div>
					</div>
...
export default {
	...
	setup(props) {
		...
		const form = useForm({
			...
			description_field_one: props.blog.description_field_one,
			description_field_two: props.blog.description_field_two,
			description_field_three: props.blog.description_field_three,
			description_field_four: props.blog.description_field_four,
			link: props.blog.link,
			category_id: props.blog.category.id,
			thumbnail: null,
			images: [],
			deleted_image: [],
		})
		...
		function submit() {
			form.post(route("app.blogs.update", props.blog.id))
		}
		...
		const deletedImage = (url) => {
			let image = props.blog.images.find((image) => image.path == url)
			if (image) {
				form.deleted_image.push(image.id)
			}
		}
		return {
			form,
