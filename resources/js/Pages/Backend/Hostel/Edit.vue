...
export default {
	...
	setup(props) {
        const thumbnail = ref(props.hostel.thumbnail)
		const form = useForm({
            _method: "PUT",
			title: props.hostel.title,
			owner: props.hostel.owner,
			owner_num: props.hostel.owner_num,
			type: props.hostel.type,
			person: props.hostel.person,
			area: props.hostel.area,
			address: props.hostel.address,
			rent: props.hostel.rent,
			thumbnail: null,
        ...
        function submit() {
			form.post(route("app.hostels.update", props.hostel.id))
		}
